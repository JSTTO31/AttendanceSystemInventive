<template>
  <v-card flat class="rounded-lg border h-100">
      <v-card color="primary" class=" text-subtitle-1 font-weight-bold rounded-0 d-none d-md-block">
        <v-row>
      ...
      <v-card class="pa-2 rounded-0" flat>
        <v-row v-for="student in students" :key="student.id">
          <v-col class=" font-weight-medium text-capitalize pa-5 d-none d-md-block" style="width: 1.714286%;">{{ student.first_name[0] }}. {{ student.last_name }}</v-col>
          <v-col class=" font-weight-medium text-capitalize pa-5 d-block d-md-none" cols="12">{{ student.first_name }} {{ student.last_name }}</v-col>
