<template>
  <v-menu open-on-hover>
    <template #activator="{props}">
      <v-card v-bind="props"  flat :color="attributes.color" :variant="attributes.variant"  class="mr-2 my-2 d-flex align-center justify-center"  height="51" style="user-select: none" width="51">{{ day }}</v-card>
    </template>
    <v-card width="380" class="pa-3 rounded-lg" style="user-select: none">
       ...
       <div class="d-flex text-subtitle-2 align-center justify-sm-space-between">
        ...
        <v-chip :color="status == 'absent' ? 'error' : status == 'present' ? 'primary' : 'warning'" class="text-capitalize" size="small" >{{ status }}</v-chip>
       </div>
    </v-card>
  </v-menu>
</template>
...
const status = computed(() => attendance.value?.is_absent ? 'absent' : attendance.value?.time_in && attendance.value.time_out ? 'present' : 'pending'  )
</script>
